...
fun ApprovedScreen(
    ...
    approvedViewModel: ApprovedViewModel = hiltViewModel()
) {
    val context = LocalContext.current

    val approvedUiState by approvedViewModel.approvedUiState.collectAsState()

    if (approvedUiState.loading) {
        LoadingItem(modifier = modifier.fillMaxSize().background(color = colorResource(R.color.white)))
    }

    approvedUiState.errorMessage?.let { error ->
        ErrorItem(message = error, modifier = modifier.fillMaxSize().background(color = colorResource(R.color.white)))
    }

    approvedUiState.approvedList?.let { ordersList ->
        if (ordersList.isNotEmpty()) {
            LazyColumn(
                modifier = modifier.fillMaxSize().background(color = colorResource(R.color.white))
            ) {
                items(items = ordersList, key = {it.documentId}) { ordersModel ->
                    OrdersCard(
